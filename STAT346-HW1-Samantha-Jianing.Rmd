...
The linearity assumption is not met because the scatterplot has a curved pattern and the residual plot fans out. Equal variance is not met because the residual plot fans out. Neearnly normal condition is not met because histogram of residuals is skewed to the right.
...
par(mfrow=c(2,2))
plot(bigmac$EngSal, (bigmac$BigMac)^(-1/2), xlab="Average Engineer's Salary", ylab="Price of BigMac")
abline(lm.adj, col="red")
hist(lm.adj$residuals, main="Histogram of Residuals", xlab="Residuals")
plot(lm.adj$fitted.values, lm.adj$residuals, xlab="Predicted Price of BigMac", ylab="Residuals", main="Residual Plot")
